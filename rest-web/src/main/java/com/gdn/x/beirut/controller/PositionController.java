...
//        requestId, filename, username, additionalParameterMap
    ...
    Position currentPosition = positionService.getPosition(storeId, idPosition);
    if (currentPosition == null) {
      throw new ApplicationException((ErrorCategory.DATA_NOT_FOUND),
          "Position with given id is not present");
    }
    PositionDescription newPositionDesc = new PositionDescription();
    ...
    if (resultPosition == null) {
      throw new ApplicationException(ErrorCategory.UNSPECIFIED, "resultPosition is not available");
    }
    return new GdnBaseRestResponse(requestId);
  }
}
